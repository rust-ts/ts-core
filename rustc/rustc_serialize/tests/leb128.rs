...
macro_rules! impl_test_unsigned_leb128 {
  ($test_name:ident, $write_fn_name:ident, $read_fn_name:ident, $int_ty:ident) => {
    ...
    fn $test_name() {
      ...
      values
        .extend((-500..500).map(|i| (i as $int_ty).wrapping_mul(0x12345789ABCDEFu64 as $int_ty)));
...
macro_rules! impl_test_signed_leb128 {
  ($test_name:ident, $write_fn_name:ident, $read_fn_name:ident, $int_ty:ident) => {
    ...
    fn $test_name() {
      ...
      values
        .extend((-500..500).map(|i| (i as $int_ty).wrapping_mul(0x12345789ABCDEFi64 as $int_ty)));
