...
pub mod NodeFlags {
  ...
  pub(crate) const PossiblyContainsDynamicImport: u32 = 1 << 20;
  pub(crate) const PossiblyContainsImportMeta: u32 = 1 << 21;

  pub const JSDoc: u32 = 1 << 22;                   // If node was parsed inside jsdoc
  pub(crate) const Ambient: u32 = 1 << 23;          // If node was inside an ambient context -- a declaration file; or inside something with the `declare` modifier.
  pub(crate) const InWithStatement: u32 = 1 << 24;  // If any ancestor of node was the `statement` of a WithStatement (not the `expression`)
  pub(crate) const JsonFile: u32 = 1 << 25;         // If node was parsed in a Json
  pub(crate) const TypeCached: u32 = 1 << 26;       // If a type was cached for node at any point
  pub(crate) const Deprecated: u32 = 1 << 27;       // If has '@deprecated' JSDoc tag
  ...
  pub(crate) const PermanentlySetIncrementalFlags: u32 =
