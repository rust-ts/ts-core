...
impl Span {
  ...
  pub fn from_inner(self, inner: InnerSpan) -> Span {
    let span = self.data();
    Span::new(span.lo + BytePos::from_usize(inner.start), span.lo + BytePos::from_usize(inner.end))
  }
}
...
impl fmt::Debug for SpanData {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    (*SPAN_DEBUG)(Span::new(self.lo, self.hi), f)
  }
}
...
#[derive(
  Debug, Eq, PartialEq, Clone, Ord, PartialOrd, Hash, HashStable_Generic, Decodable, Encodable,
)]
...
impl RealFileName {
  ...
  pub fn local_path(&self) -> &Path {
    match self {
      RealFileName::Named(p) | RealFileName::Devirtualized { local_path: p, virtual_name: _ } => &p,
    }
  }
  ...
  pub fn stable_name(&self) -> &Path {
    match self {
      RealFileName::Named(p) | RealFileName::Devirtualized { local_path: _, virtual_name: p } => &p,
    }
  }
}
...
impl SourceFile {
    ...
    F: FnOnce() -> Option<String>,
  {
    if matches!(
      *self.external_src.borrow(),
      ...
      if let ExternalSource::Foreign { kind: src_kind @ ExternalSourceKind::AbsentOk, .. } =
        &mut *external_src
      {
  ...
  pub fn lookup_file_pos_with_col_display(&self, pos: BytePos) -> (usize, CharPos, usize) {
    ...
    if line > 0 {
      ...
      let col_display = {
        let start_width_idx =
          self.non_narrow_chars.binary_search_by_key(&linebpos, |x| x.pos()).unwrap_or_else(|x| x);
        let end_width_idx =
          self.non_narrow_chars.binary_search_by_key(&pos, |x| x.pos()).unwrap_or_else(|x| x);
        let special_chars = end_width_idx - start_width_idx;
        let non_narrow: usize =
          self.non_narrow_chars[start_width_idx..end_width_idx].iter().map(|x| x.width()).sum();
