...
#[inline]
pub fn original_sp(sp: Span, _enclosing_sp: Span) -> Span {
  sp
}
...
impl SourceMap {
  ...
  fn try_new_source_file(
    ...
    if let FileName::Real(real_filename) = &mut filename {
      match real_filename {
        ...
        | RealFileName::Devirtualized { local_path: path_to_be_remapped, virtual_name: _ } => {
  ...
  pub fn generate_fn_name_span(&self, span: Span) -> Option<Span> {
    ...
    if let Ok(snippet) = self.span_to_snippet(prev_span) {
      ...
      let len =
        snippet.find(|c: char| !c.is_alphanumeric() && c != '_').expect("no label after fn");
  ...
  pub fn generate_local_type_param_snippet(&self, span: Span) -> Option<(Span, String)> {
    ...
    if sugg_span != span {
      if let Ok(snippet) = self.span_to_snippet(sugg_span) {
        ...
        new_snippet.push_str(&self.span_to_snippet(span).unwrap_or_else(|_| "T".to_string()));
        new_snippet.push('>');
